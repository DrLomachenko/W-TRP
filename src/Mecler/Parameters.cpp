...
void Parameters::readFile(const string &file) {
    ...
    zeroBlocksFunction = 0;
    jobsToolsMatrixForIga.clear();
    for (auto line : jobsToolsMatrix) {
        std::vector<unsigned int> lineCompact;
        for (int t = 0; t < numTools; ++t)
        {
            if (line[t] == 1) {
                lineCompact.push_back(t);
            }
        }
        jobsToolsMatrixForIga.push_back(lineCompact);
    }
    inFile.close();
}
